...
class Model(object):
    ...
    def __init__(self):
        ...
        self.train_data_path = os.path.join(self.path, train_file)
        self.predict_data_path = os.path.join(self.path, predict_data_file)
    ...
    def output_result(self):
        ...
        result = []
        with open(self.predict_data_path, encoding='utf-8') as f:
            for line in f.readlines():
                line_dict = line.strip().split('\t')
                if len(line_dict) >= 2:  # 确保行分割正确
                    uid = line_dict[0]
                    mid = line_dict[1]
                    predict = model.get(uid, "0,0,0")
                    result.append("%s\t%s\t%s\n" % (uid, mid, predict))
